 ...
 * Jetty 9.x {@link org.eclipse.jetty.websocket.api.Session web socket} implementation.
 *
 * @since 6.2
 */
public class PatchedJetty9WebSocketProcessor extends AbstractWebSocketProcessor
		implements
		WebSocketListener {

	private static final Logger LOG = LoggerFactory.getLogger(PatchedJetty9WebSocketProcessor.class);
	 ...
	 */
	public PatchedJetty9WebSocketProcessor(final UpgradeRequest upgradeRequest,
			final UpgradeResponse upgradeResponse, final WebApplication application) {
		super(new PatchedJetty9UpgradeHttpRequest(upgradeRequest), application);
	}
	...
	public void onOpen(Object connection) {
		if (!(connection instanceof Session)) {
			throw new IllegalArgumentException(PatchedJetty9WebSocketProcessor.class.getName() +
